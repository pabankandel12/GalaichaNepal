...
if (isset($_POST['submit1'])) {
    ...
    // Confirm password
    if ($password !== $cpassword) {
        echo "<script>alert('Password doesn’t match!'); window.location='index.php';</script>";
        exit;
    }
    ...
    $file_tmp  = $_FILES['profileImage']['tmp_name'];
    if (!empty($file_name)) {
        move_uploaded_file($file_tmp, "user-image/" . $file_name);
    } else {
        $file_name = ""; // default blank
    }

    // Insert into DB
    $sql = "INSERT INTO user (name, username, email, phone, password, pro_image, address) 
            VALUES ('$Fullname', '$username', '$email', '$number', '$encriptionPassword', '$file_name', '$address')";
    if (mysqli_query($conn, $sql)) {
        header("Location: index.php");
        exit;
    } else {
...
// -------------------- LOGIN --------------------
if (isset($_POST['submit'])) {
    $username = $_POST['email'];   // actually email
    $password = $_POST['password'];
    ...
    if (!empty($username) && !empty($password)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if (password_verify($password, $row['password'])) {
                $_SESSION['Normal_username'] = $row['username'];
